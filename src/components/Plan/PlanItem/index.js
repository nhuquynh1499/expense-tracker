...
function PlanItem(props) {
  ...
  function formatDay(date) {
    ...
    return (
      d.getDate() + " " + monthOfYear[d.getMonth()] + " " + d.getFullYear()
    );
  }
  ...
  function dayFromNow() {
    const dStart = new Date(dateStart);
    return Date.now() - dStart > 0
      ? Math.ceil((Date.now() - dStart) / (3600000 * 24))
      : 0;
  }
  ...
  return (
    <div className="planItem">
      <div className="headerPlan">
        ...
        <div className="number">
          ...
          <span className="amount">
            Còn lại {Number(goal + amount).toLocaleString()}
          </span>
        </div>
      </div>
      <LinearProgress
      ...
      <div className="time">
        <p>
          {formatDay(dateStart)} - {formatDay(dateEnd)}
        </p>
        {new Date().getDate() > dateEnd ? (
          <p>
            {dayFromNow()} / {sumDay()}
          </p>
        ) : (
          amount < goal ? <p className="pass">Hoàn thành xuất sắc</p> : <p className="fail">Vượt chỉ tiêu</p>
        )}
      </div>
    </div>
  );
